#!/usr/bin/env bun
/**
 * 🧪 Direct HTTP Production Test Script for Instagram Functions
 * Tests all functions with emoji names using direct HTTP requests
 */

const API_URL = 'http://localhost:4000'

console.log('🚀 DIRECT HTTP PRODUCTION TESTING SCRIPT')
...
// Direct HTTP request function
async function sendEventDirectly(eventName: string, data: any) {
  const eventId = `prod-test-${Date.now()}-${Math.random()
    .toString(36)
    .substr(2, 9)}`
...
async function runProductionTest() {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      const { eventId, result } = await sendEventDirectly(
        testCase.eventName,
        testCase.data
      )
