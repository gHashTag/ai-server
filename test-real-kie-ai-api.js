...
class RealKieAiTest {
  ...
  async testAccountBalance() {
    ...
    try {
      ...
      if (credits > 0) {
        ...
        console.log(`📊 Можно сгенерировать ~${maxDuration} секунд Veo-3 Fast видео`);
  ...
  async testVideoGeneration() {
    ...
    if (!shouldContinue) {
      console.log('🛑 Реальный тест пропущен (добавьте RUN_REAL_TESTS=true для запуска)');
    ...
    try {
      ...
      if (response.data.success && response.data.data?.videoUrl) {
        ...
        console.log(`⏱️  Duration: ${response.data.data.duration || 'unknown'} секунд`);
  ...
  async testDurationAPI() {
    ...
    const testCases = [
      ...
      { duration: 15, expected: 10, description: 'Превышение лимита (должно стать 10)' }
    ];
    ...
    testCases.forEach(testCase => {
      ...
      console.log(`   ${isCorrect ? '✅' : '❌'} ${isCorrect ? 'КОРРЕКТНО' : 'ОШИБКА'}`);
    });
  }
}
