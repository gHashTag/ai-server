import { TranscriptionAsset } from '@/interfaces/supabase.interface'
import { supabase } from './index'

export const setRoomAsset = async (roomAsset: TranscriptionAsset) => {
  try {
    ...
    if (errorInsertRoomAsset) {
      throw new Error(`Asset creation failed: ${errorInsertRoomAsset.message}`)
    }
