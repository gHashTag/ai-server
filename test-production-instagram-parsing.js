...
async function testInstagramParsing() {
  ...
  try {
    ...
    const healthCheck = await axios.get(`${PRODUCTION_URL}/health`).catch(err => ({ 
      status: err.response?.status,
      data: err.response?.data 
    }));
    console.log(`   ✅ Сервер отвечает: ${healthCheck.status || healthCheck.data?.status || 'OK'}\n`);

    // 2. Проверка доступности Inngest эндпоинта
    console.log('2️⃣ Проверка Inngest эндпоинта...');
    const inngestCheck = await axios.get(`${PRODUCTION_URL}/api/inngest`).catch(err => ({
      status: err.response?.status,
      statusText: err.response?.statusText
    }));
    console.log(`   ${inngestCheck.status === 200 ? '✅' : '⚠️'} Inngest эндпоинт: ${inngestCheck.status} ${inngestCheck.statusText || ''}\n`);
    ...
    if (parseResult) {
      ...
      console.log('Результат:', JSON.stringify(parseResult, null, 2).substring(0, 500) + '...');
    } else {
    ...
    try {
      ...
      if (functionsResponse.data) {
        ...
        const hasInstagramScraper = text.includes('instagram') || text.includes('scraper');
        console.log(`   ${hasInstagramScraper ? '✅' : '❌'} Instagram Scraper функция ${hasInstagramScraper ? 'найдена' : 'не найдена'}`);
      }
    } catch (err) {
    ...
    for (const endpoint of systemEndpoints) {
      try {
        const response = await axios.get(`${PRODUCTION_URL}${endpoint}`, { timeout: 5000 });
    ...
    console.log(parseResult ? '✅ Instagram парсинг функционирует' : '⚠️ Требуется проверка через Inngest Dev Server');
    console.log('='.repeat(60));
