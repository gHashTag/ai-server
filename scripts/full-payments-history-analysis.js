...
function displayFullHistoryReport(botStats, monthlyStats) {
  ...
  const sortedMonths = Array.from(monthlyStats.values()).sort((a, b) =>
  ...
  sortedMonths.forEach(month => {
    console.log(`\n📆 ${month.month}:`)
    console.log(
      `   💰 ДОХОДЫ - Реальные: ${month.total_income_real_rub.toFixed(
        2
      )} RUB | ${month.total_income_real_stars.toFixed(
        0
      )} STARS | Виртуальные: ${month.total_income_virtual.toFixed(0)}`
    )
    console.log(
      `   💸 РАСХОДЫ - Реальные: ${month.total_expense_real_rub.toFixed(
        2
      )} RUB | ${month.total_expense_real_stars.toFixed(
        0
      )} STARS | Виртуальные: ${month.total_expense_virtual.toFixed(0)}`
    )
    console.log(
      `   🤖 Ботов: ${month.bots_count} | 📋 Транзакций: ${month.transactions_count}`
    )
  })
  ...
  sortedBots.forEach((stats, index) => {
    ...
    console.log(
      `   💰 ДОХОДЫ: ${stats.total_income_real_rub.toFixed(
        2
      )} RUB | ${stats.total_income_real_stars.toFixed(
        0
      )} STARS | Виртуальные: ${stats.total_income_virtual.toFixed(0)}`
    )
    console.log(
      `   💸 РАСХОДЫ: ${stats.total_expense_real_rub.toFixed(
        2
      )} RUB | ${stats.total_expense_real_stars.toFixed(
        0
      )} STARS | Виртуальные: ${stats.total_expense_virtual.toFixed(0)}`
    )
    console.log(
      `   📊 ЧИСТАЯ ПРИБЫЛЬ: ${(
        stats.total_income_real_rub - stats.total_expense_real_rub
      ).toFixed(2)} RUB | ${(
        stats.total_income_real_stars - stats.total_expense_real_stars
      ).toFixed(0)} STARS`
    )
    console.log(
      `   📋 Всего транзакций: ${stats.transactions_count} | Месяцев активности: ${stats.months.size}`
    )
  })

  // Общие итоги
  const totalIncomeRealRub = sortedBots.reduce(
    (sum, bot) => sum + bot.total_income_real_rub,
    0
  )
  const totalIncomeRealStars = sortedBots.reduce(
    (sum, bot) => sum + bot.total_income_real_stars,
    0
  )
  const totalIncomeVirtual = sortedBots.reduce(
    (sum, bot) => sum + bot.total_income_virtual,
    0
  )
  const totalExpenseRealRub = sortedBots.reduce(
    (sum, bot) => sum + bot.total_expense_real_rub,
    0
  )
  const totalExpenseRealStars = sortedBots.reduce(
    (sum, bot) => sum + bot.total_expense_real_stars,
    0
  )
  const totalExpenseVirtual = sortedBots.reduce(
    (sum, bot) => sum + bot.total_expense_virtual,
    0
  )
  const totalTransactions = sortedBots.reduce(
    (sum, bot) => sum + bot.transactions_count,
    0
  )
  ...
  console.log('\n💰 ОБЩИЕ ДОХОДЫ:')
  console.log(
    `   Реальные: ${totalIncomeRealRub.toFixed(
      2
    )} RUB | ${totalIncomeRealStars.toFixed(0)} STARS`
  )
  console.log(`   Виртуальные: ${totalIncomeVirtual.toFixed(0)} единиц`)
  console.log('\n💸 ОБЩИЕ РАСХОДЫ:')
  console.log(
    `   Реальные: ${totalExpenseRealRub.toFixed(
      2
    )} RUB | ${totalExpenseRealStars.toFixed(0)} STARS`
  )
  console.log(`   Виртуальные: ${totalExpenseVirtual.toFixed(0)} единиц`)
  console.log('\n📊 ЧИСТАЯ ПРИБЫЛЬ:')
  console.log(
    `   ${(totalIncomeRealRub - totalExpenseRealRub).toFixed(2)} RUB | ${(
      totalIncomeRealStars - totalExpenseRealStars
    ).toFixed(0)} STARS`
  )
  console.log('='.repeat(100))
}
...
function createFullHistoryExcel(botStats, monthlyStats, allTransactions) {
  try {
    ...
    monthlyData.push([
      'МЕСЯЦ',
      'ДОХОДЫ РЕАЛЬНЫЕ RUB',
      'ДОХОДЫ РЕАЛЬНЫЕ STARS',
      'ДОХОДЫ ВИРТУАЛЬНЫЕ',
      'РАСХОДЫ РЕАЛЬНЫЕ RUB',
      'РАСХОДЫ РЕАЛЬНЫЕ STARS',
      'РАСХОДЫ ВИРТУАЛЬНЫЕ',
      'БОТОВ АКТИВНЫХ',
      'ТРАНЗАКЦИЙ',
    ])
    ...
    sortedBots.forEach(stats => {
      botsData.push([
        ...
        parseFloat(
          (stats.total_income_real_rub - stats.total_expense_real_rub).toFixed(
            2
          )
        ),
        parseFloat(
    ...
    sortedBots.forEach(botStats => {
      const sortedBotMonths = Array.from(botStats.months.values()).sort(
...
async function main() {
  ...
  try {
    ...
    const excelFile = createFullHistoryExcel(
      botStats,
      monthlyStats,
      allTransactions
    )
