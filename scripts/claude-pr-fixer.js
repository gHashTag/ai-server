...
class ClaudePRFixer {
  ...
  async fixPR(prNumber) {
    ...
    try {
      ...
      console.log(`❌ Найдено ${failedChecks.length} проблем:`, failedChecks.map(c => c.name));
  ...
  async getPRInfo(prNumber) {
    const result = execSync(`gh pr view ${prNumber} --json title,body,headRefName`, 
      { encoding: 'utf8' }
    );
...
${context.files.slice(0, 10).map(f => `- ${f}`).join('\n')}
${context.files.length > 10 ? `... и еще ${context.files.length - 10} файлов` : ''}
...
`;
  ...
  getBasicFixes(context) {
    ...
    if (context.failedChecks.some(c => c.name.includes('typescript') || c.name.includes('build'))) {
      changes.push({
        file: 'package.json',
        action: 'edit',
  ...
  async applyFixes(changes, prNumber) {
    ...
    for (const change of changes) {
      try {
        ...
        console.log(`⚠️ Не удалось применить изменение для ${change.file}: ${error.message}`);
      }
    }
