  ...
  try {
    ...
    ;[...virtualIncomes, ...virtualExpenses].forEach(payment => {
      ...
      if (payment.type === 'MONEY_INCOME') {
        stats.total_virtual_income += parseFloat(
          payment.amount || payment.stars || 0
        )
      } else {
        stats.total_virtual_expense += parseFloat(
          payment.amount || payment.stars || 0
        )
      }
...
function displayFullReport(botsAnalysis) {
  ...
  sortedBots.forEach((stats, index) => {
    const hasRealIncome =
      stats.total_real_income_rub > 0 || stats.total_real_income_stars > 0
    const hasRealExpense =
      stats.total_real_expense_rub > 0 || stats.total_real_expense_stars > 0
    ...
    if (hasAnyReal) {
      if (hasRealIncome) {
        console.log(
          `   💰 РЕАЛЬНЫЕ ДОХОДЫ: ${stats.total_real_income_rub.toFixed(
            2
          )} RUB | ${stats.total_real_income_stars.toFixed(2)} STARS (${
      ...
      if (hasRealExpense) {
        console.log(
          `   💸 РЕАЛЬНЫЕ РАСХОДЫ: ${stats.total_real_expense_rub.toFixed(
            2
          )} RUB | ${stats.total_real_expense_stars.toFixed(2)} STARS (${
            stats.real_expense_transactions
          } тр.)`
        )
        botsWithExpense++
      }
      ...
      const netStars =
        stats.total_real_income_stars - stats.total_real_expense_stars
      console.log(
        `   📊 ЧИСТАЯ ПРИБЫЛЬ: ${netRub.toFixed(2)} RUB | ${netStars.toFixed(
          2
        )} STARS`
      )
    } else {
  ...
  console.log(
    `\n💵 ОБЩИЕ РЕАЛЬНЫЕ ДОХОДЫ: ${totalRealIncomeRub.toFixed(
      2
    )} RUB | ${totalRealIncomeStars.toFixed(2)} STARS`
  )
  console.log(
    `💸 ОБЩИЕ РЕАЛЬНЫЕ РАСХОДЫ: ${totalRealExpenseRub.toFixed(
      2
    )} RUB | ${totalRealExpenseStars.toFixed(2)} STARS`
  )
  console.log(
    `📊 ЧИСТАЯ ПРИБЫЛЬ: ${(totalRealIncomeRub - totalRealExpenseRub).toFixed(
      2
    )} RUB | ${(totalRealIncomeStars - totalRealExpenseStars).toFixed(2)} STARS`
  )
  console.log('='.repeat(80))
}
  ...
  try {
    ...
    sortedBots.forEach(stats => {
      summaryData.push([
        ...
        parseFloat(
          (stats.total_real_income_rub - stats.total_real_expense_rub).toFixed(
            2
          )
        ),
        parseFloat(
    ...
    profitableData.push([
      'БОТ',
      'РЕАЛЬНЫЕ ДОХОДЫ RUB',
      'РЕАЛЬНЫЕ ДОХОДЫ STARS',
      'ТРАНЗАКЦИЙ',
      'УСЛОВНАЯ СТОИМОСТЬ RUB',
    ])

    const profitableBots = sortedBots.filter(
      bot => bot.total_real_income_rub > 0 || bot.total_real_income_stars > 0
    )
    ...
    profitableBots.forEach(stats => {
      profitableData.push([
        ...
        parseFloat(
          (
            stats.total_real_income_rub +
            stats.total_real_income_stars * 0.5
          ).toFixed(2)
        ),
      ])
    })
