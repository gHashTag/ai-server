...
async function getAllBotsAndPayments() {
  try {
    ...
    console.log(`   📋 Доходы с ботами: ${incomesWithBots.length}, без ботов: ${incomesWithoutBots.length}`);
    console.log(`   📋 Расходы с ботами: ${expensesWithBots.length}, без ботов: ${expensesWithoutBots.length}`);
...
function analyzeThreeMonths(data) {
  ...
  uniqueBots.forEach(botName => {
    botStats.set(botName, {
      bot_name: botName,
      may: { income_rub: 0, income_stars: 0, income_virtual: 0, expense_rub: 0, expense_stars: 0, expense_virtual: 0, transactions: 0 },
      june: { income_rub: 0, income_stars: 0, income_virtual: 0, expense_rub: 0, expense_stars: 0, expense_virtual: 0, transactions: 0 },
      july: { income_rub: 0, income_stars: 0, income_virtual: 0, expense_rub: 0, expense_stars: 0, expense_virtual: 0, transactions: 0 },
      total_income_rub: 0,
  ...
  const monthlyTotals = {
    may: { income_rub: 0, income_stars: 0, income_virtual: 0, expense_rub: 0, expense_stars: 0, expense_virtual: 0, transactions: 0, active_bots: new Set() },
    june: { income_rub: 0, income_stars: 0, income_virtual: 0, expense_rub: 0, expense_stars: 0, expense_virtual: 0, transactions: 0, active_bots: new Set() },
    july: { income_rub: 0, income_stars: 0, income_virtual: 0, expense_rub: 0, expense_stars: 0, expense_virtual: 0, transactions: 0, active_bots: new Set() }
  };
  ...
  incomes.forEach(payment => {
    ...
    stats.all_transactions.push({ ...payment, month: monthKey, is_real: isReal });
  });
...
function displayCorrectReport(botStats, monthlyTotals) {
  ...
  months.forEach(month => {
    console.log(`\n🗓️  ${month.name}:`);
    console.log(`   💰 ДОХОДЫ:`);
    console.log(`      Реальные: ${month.data.income_rub.toFixed(2)} RUB | ${month.data.income_stars.toFixed(0)} STARS`);
    console.log(`      Виртуальные: ${month.data.income_virtual.toFixed(0)} единиц`);
    console.log(`   💸 РАСХОДЫ:`);
    console.log(`      Реальные: ${month.data.expense_rub.toFixed(2)} RUB | ${month.data.expense_stars.toFixed(0)} STARS`);
    console.log(`      Виртуальные: ${month.data.expense_virtual.toFixed(0)} единиц`);
    console.log(`   📊 АКТИВНОСТЬ: ${month.data.active_bots} ботов | ${month.data.transactions} транзакций`);
  });
  ...
  sortedBots.forEach((stats, index) => {
    ...
    const hasRealMoney = stats.total_income_rub > 0 || stats.total_income_stars > 0 || stats.total_expense_rub > 0 || stats.total_expense_stars > 0;
    
    console.log(`\n${index + 1}. 🤖 ${stats.bot_name} ${hasRealMoney ? '💰' : hasActivity ? '👻' : '😴'}`);
    
    if (hasActivity) {
      // Показываем по месяцам
      console.log(`   📅 МАЙ: ${stats.may.income_rub.toFixed(0)}₽+${stats.may.income_stars.toFixed(0)}⭐ доход | ${stats.may.expense_rub.toFixed(0)}₽+${stats.may.expense_stars.toFixed(0)}⭐ расход | ${stats.may.transactions} тр`);
      console.log(`   📅 ИЮНЬ: ${stats.june.income_rub.toFixed(0)}₽+${stats.june.income_stars.toFixed(0)}⭐ доход | ${stats.june.expense_rub.toFixed(0)}₽+${stats.june.expense_stars.toFixed(0)}⭐ расход | ${stats.june.transactions} тр`);
      console.log(`   📅 ИЮЛЬ: ${stats.july.income_rub.toFixed(0)}₽+${stats.july.income_stars.toFixed(0)}⭐ доход | ${stats.july.expense_rub.toFixed(0)}₽+${stats.july.expense_stars.toFixed(0)}⭐ расход | ${stats.july.transactions} тр`);
      ...
      if (hasRealMoney) {
        ...
        console.log(`   📊 ИТОГО РЕАЛЬНЫХ: ${stats.total_income_rub.toFixed(2)}₽ - ${stats.total_expense_rub.toFixed(2)}₽ = ${netRub.toFixed(2)}₽ | ${stats.total_income_stars.toFixed(0)}⭐ - ${stats.total_expense_stars.toFixed(0)}⭐ = ${netStars.toFixed(0)}⭐`);
      }
      
      if (stats.total_income_virtual > 0 || stats.total_expense_virtual > 0) {
        console.log(`   👻 Виртуальных: ${stats.total_income_virtual.toFixed(0)} доход | ${stats.total_expense_virtual.toFixed(0)} расход`);
      }
    } else {
  ...
  // Общие итоги
  const totalIncomeRub = sortedBots.reduce((sum, bot) => sum + bot.total_income_rub, 0);
  const totalIncomeStars = sortedBots.reduce((sum, bot) => sum + bot.total_income_stars, 0);
  const totalExpenseRub = sortedBots.reduce((sum, bot) => sum + bot.total_expense_rub, 0);
  const totalExpenseStars = sortedBots.reduce((sum, bot) => sum + bot.total_expense_stars, 0);
  const totalTransactions = sortedBots.reduce((sum, bot) => sum + bot.total_transactions, 0);
  const botsWithActivity = sortedBots.filter(bot => bot.total_transactions > 0).length;
  const botsWithRealMoney = sortedBots.filter(bot => 
    bot.total_income_rub > 0 || bot.total_income_stars > 0 || 
    bot.total_expense_rub > 0 || bot.total_expense_stars > 0
  ).length;
  ...
  console.log('\n💰 РЕАЛЬНЫЕ ДЕНЬГИ:');
  console.log(`   Доходы: ${totalIncomeRub.toFixed(2)} RUB | ${totalIncomeStars.toFixed(0)} STARS`);
  console.log(`   Расходы: ${totalExpenseRub.toFixed(2)} RUB | ${totalExpenseStars.toFixed(0)} STARS`);
  console.log(`   ЧИСТАЯ ПРИБЫЛЬ: ${(totalIncomeRub - totalExpenseRub).toFixed(2)} RUB | ${(totalIncomeStars - totalExpenseStars).toFixed(0)} STARS`);
  console.log('='.repeat(100));
}
...
function createCorrectExcel(botStats, monthlyTotals) {
  try {
    ...
    botsData.push([
      'БОТ',
      'МАЙ ДОХОД RUB', 'МАЙ ДОХОД STARS', 'МАЙ РАСХОД RUB', 'МАЙ РАСХОД STARS', 'МАЙ ТРАНЗАКЦИЙ',
      'ИЮНЬ ДОХОД RUB', 'ИЮНЬ ДОХОД STARS', 'ИЮНЬ РАСХОД RUB', 'ИЮНЬ РАСХОД STARS', 'ИЮНЬ ТРАНЗАКЦИЙ',
      'ИЮЛЬ ДОХОД RUB', 'ИЮЛЬ ДОХОД STARS', 'ИЮЛЬ РАСХОД RUB', 'ИЮЛЬ РАСХОД STARS', 'ИЮЛЬ ТРАНЗАКЦИЙ',
      'ИТОГО ДОХОД RUB', 'ИТОГО ДОХОД STARS', 'ИТОГО РАСХОД RUB', 'ИТОГО РАСХОД STARS',
      'ЧИСТАЯ ПРИБЫЛЬ RUB', 'ЧИСТАЯ ПРИБЫЛЬ STARS', 'ВСЕГО ТРАНЗАКЦИЙ'
    ]);
    ...
    sortedBots.forEach(stats => {
      botsData.push([
        stats.bot_name,
        stats.may.income_rub, stats.may.income_stars, stats.may.expense_rub, stats.may.expense_stars, stats.may.transactions,
        stats.june.income_rub, stats.june.income_stars, stats.june.expense_rub, stats.june.expense_stars, stats.june.transactions,
        stats.july.income_rub, stats.july.income_stars, stats.july.expense_rub, stats.july.expense_stars, stats.july.transactions,
        stats.total_income_rub, stats.total_income_stars, stats.total_expense_rub, stats.total_expense_stars,
        stats.total_income_rub - stats.total_expense_rub, stats.total_income_stars - stats.total_expense_stars,
    ...
    // 2. Сводка по месяцам
    const monthData = [];
    monthData.push(['МЕСЯЦ', 'ДОХОД RUB', 'ДОХОД STARS', 'РАСХОД RUB', 'РАСХОД STARS', 'БОТОВ АКТИВНЫХ', 'ТРАНЗАКЦИЙ']);
    
    monthData.push(['МАЙ 2025', monthlyTotals.may.income_rub, monthlyTotals.may.income_stars, 
                   monthlyTotals.may.expense_rub, monthlyTotals.may.expense_stars, 
                   monthlyTotals.may.active_bots, monthlyTotals.may.transactions]);
    monthData.push(['ИЮНЬ 2025', monthlyTotals.june.income_rub, monthlyTotals.june.income_stars, 
                   monthlyTotals.june.expense_rub, monthlyTotals.june.expense_stars, 
                   monthlyTotals.june.active_bots, monthlyTotals.june.transactions]);
    monthData.push(['ИЮЛЬ 2025', monthlyTotals.july.income_rub, monthlyTotals.july.income_stars, 
                   monthlyTotals.july.expense_rub, monthlyTotals.july.expense_stars, 
                   monthlyTotals.july.active_bots, monthlyTotals.july.transactions]);
    ...
    // 3. Все транзакции
    const transData = [];
    transData.push(['БОТ', 'ДАТА', 'МЕСЯЦ', 'ТИП', 'РЕАЛЬНЫЙ', 'RUB', 'STARS', 'ВАЛЮТА', 'МЕТОД', 'ОПИСАНИЕ']);
    ...
    allTransactions
      ...
      .forEach(t => {
        transData.push([
          t.bot_name, t.payment_date, t.month.toUpperCase(), t.type, 
          t.is_real ? 'ДА' : 'НЕТ', t.amount || 0, t.stars || 0, 
          t.currency, t.payment_method, t.description || ''
        ]);
      });
...
async function main() {
  ...
  try {
    ...
    const excelFile = createCorrectExcel(analysis.botStats, analysis.monthlyTotals);
