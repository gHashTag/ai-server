  ...
  try {
    ...
    try {
      ...
      getResult.rows.forEach((sub, index) => {
        console.log(`   ${index + 1}. @${sub.competitor_username} (${sub.max_reels} рилз, ${sub.delivery_format})`)
      })
      ...
      topResult.rows.forEach((comp, index) => {
        console.log(`   ${index + 1}. @${comp.competitor_username} (${comp.subscribers_count} подписчиков)`)
      })
      ...
      if (deleteResult.rows.length > 0) {
        console.log('✅ Подписка удалена:', deleteResult.rows[0].competitor_username)
      }
