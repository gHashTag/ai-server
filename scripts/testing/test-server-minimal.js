...
try {
  ...
  console.log('🔗 Можно тестировать: http://localhost:4000/api/competitor-subscriptions')
  ...
  setTimeout(async () => {
    try {
      ...
      try {
        const statsResponse = await axios.get('http://localhost:4000/api/competitor-subscriptions/stats')
        console.log('✅ Competitor stats:', statsResponse.status, 'Users:', statsResponse.data.stats?.total_users)
      } catch (error) {
        console.log('❌ Competitor stats failed:', error.response?.status, error.message)
      }
