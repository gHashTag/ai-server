...
async function testEndpoints() {
  ...
  try {
    ...
    try {
      ...
      console.log('❌ Ошибка получения статистики:', error.response?.status, error.message)
    }
    ...
    try {
      ...
      try {
        const getResponse = await axios.get(`${API_BASE}?user_telegram_id=12345678&bot_name=test_bot`)
        console.log('✅ Подписки получены:', {
          status: getResponse.status,
          subscriptions_count: getResponse.data.subscriptions?.length,
          first_subscription: getResponse.data.subscriptions?.[0]?.competitor_username
        })
      } catch (error) {
        console.log('❌ Ошибка получения подписок:', error.response?.status, error.message)
      }
      ...
      if (subscriptionId) {
        ...
        try {
          const updateResponse = await axios.put(`${API_BASE}/${subscriptionId}`, {
            max_reels: 8,
          ...
          console.log('❌ Ошибка обновления подписки:', error.response?.status, error.message)
        }

        // 5. Тест ручного запуска парсинга
        console.log('\n🚀 Тест 5: Ручной запуск парсинга...')
        try {
          const triggerResponse = await axios.post(`${API_BASE}/${subscriptionId}/trigger-parsing`)
          ...
          console.log('❌ Ошибка запуска парсинга:', error.response?.status, error.message)
        }

        // 6. Тест получения истории
        console.log('\n📈 Тест 6: Получение истории доставок...')
        try {
          const historyResponse = await axios.get(`${API_BASE}/${subscriptionId}/history`)
          console.log('✅ История получена:', {
            status: historyResponse.status,
            history_count: historyResponse.data.history?.length
          })
        } catch (error) {
          console.log('❌ Ошибка получения истории:', error.response?.status, error.message)
        }

        // 7. Тест удаления подписки
        console.log('\n🗑️ Тест 7: Удаление подписки...')
        try {
          const deleteResponse = await axios.delete(`${API_BASE}/${subscriptionId}`)
          console.log('✅ Подписка удалена:', {
            status: deleteResponse.status,
            message: deleteResponse.data.message
          })
        } catch (error) {
          console.log('❌ Ошибка удаления подписки:', error.response?.status, error.message)
        }
      }
      ...
      try {
        ...
        console.log('✅ Итоговая статистика:', {
          ...
          total_subscriptions: finalStatsResponse.data.stats?.total_subscriptions,
          active_subscriptions: finalStatsResponse.data.stats?.active_subscriptions,
          unique_competitors: finalStatsResponse.data.stats?.unique_competitors
        })
      } catch (error) {
        console.log('❌ Ошибка получения итоговой статистики:', error.response?.status, error.message)
      }

    } catch (error) {
      console.log('❌ Ошибка создания подписки:', error.response?.status, error.response?.data || error.message)
    }
