...
const CORRECT_URL = 'https://ai-server-production-production-8e2d.up.railway.app'
...
async function checkCorrectServer() {
  ...
  const competitorEndpoints = [
    { name: 'Stats', path: '/api/competitor-subscriptions/stats', method: 'GET' },
    { name: 'List', path: '/api/competitor-subscriptions?user_telegram_id=test&bot_name=test', method: 'GET' },
  ]
      ...
      if (response.status < 300) {
        ...
        if (response.data?.success) {
          ...
          if (response.data.stats) {
            console.log(`      📈 Stats: Users=${response.data.stats.total_users}, Subs=${response.data.stats.total_subscriptions}`)
          }
        }
  ...
  const videoEndpoints = [
    ...
    { name: 'Video Status Example', path: '/generate/text-to-video/status/test123' },
  ]
  ...
  try {
    const createResponse = await axios.post(`${CORRECT_URL}/api/competitor-subscriptions`, {
    ...
    if (createResponse.status < 300) {
      ...
      if (createResponse.data?.subscription?.id) {
        ...
        try {
          const deleteResponse = await axios.delete(`${CORRECT_URL}/api/competitor-subscriptions/${subscriptionId}`, { 
            timeout: 5000,
            validateStatus: () => true 
          })
      ...
      if (createResponse.data) {
        console.log(`   📊 Ответ:`, JSON.stringify(createResponse.data, null, 2).substring(0, 200))
      }
    }
  ...
  try {
    const videoResponse = await axios.post(`${CORRECT_URL}/generate/text-to-video`, {
    ...
    if (videoResponse.status < 300 && videoResponse.data?.jobId) {
      ...
      setTimeout(async () => {
        try {
          const statusResponse = await axios.get(`${CORRECT_URL}/generate/text-to-video/status/${videoResponse.data.jobId}`, {
            timeout: 5000,
            validateStatus: () => true
          })
          console.log(`   📊 Статус задания: ${statusResponse.status}`)
          if (statusResponse.data) {
            console.log(`   📈 Job status:`, JSON.stringify(statusResponse.data, null, 2).substring(0, 200))
          }
        } catch (e) {
