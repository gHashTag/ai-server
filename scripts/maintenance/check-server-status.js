...
async function checkServerStatus() {
  ...
  const endpoints = [
    ...
    { name: 'Root Endpoint', url: `${RAILWAY_URL}/` },
    {
      ...
      url: `${RAILWAY_URL}/api/competitor-subscriptions/stats`,
    },
    {
      name: 'Competitor Base',
      url: `${RAILWAY_URL}/api/competitor-subscriptions`,
    },
    { name: 'Video Generation', url: `${RAILWAY_URL}/generate/text-to-video` },
    {
      name: 'Video Status (Example)',
      url: `${RAILWAY_URL}/generate/text-to-video/status/test123`,
    },
  ]
  ...
  try {
    const createResponse = await axios.post(
      `${RAILWAY_URL}/api/competitor-subscriptions`,
      {
    ...
    if (createResponse.data) {
      console.log(
        `   📊 POST ответ:`,
        JSON.stringify(createResponse.data, null, 2).substring(0, 300) + '...'
      )
    }
    ...
    if (createResponse.status === 200 && createResponse.data.subscription?.id) {
      const deleteResponse = await axios.delete(
        `${RAILWAY_URL}/api/competitor-subscriptions/${createResponse.data.subscription.id}`,
        {
          timeout: 5000,
          validateStatus: () => true,
        }
