...
console.log(
  `   • За секунду = $${SYNX_SELLING_PRICE.price_per_second_usd.toFixed(3)} (${(
    SYNX_SELLING_PRICE.price_per_second_usd * USD_TO_RUB
  ).toFixed(1)}₽)`
)
...
console.log(
  `      • За секунду: $${OUR_COST_PRICES['veo-3-fast']} (${(
    OUR_COST_PRICES['veo-3-fast'] * USD_TO_RUB
  ).toFixed(1)}₽)`
)
console.log(
  `      • 8 секунд: $${ourFastCost8sec} = ${ourFastCostRub8sec.toFixed(0)}₽`
)
...
console.log(
  `      • За секунду: $${OUR_COST_PRICES['veo-3-quality']} (${(
    OUR_COST_PRICES['veo-3-quality'] * USD_TO_RUB
  ).toFixed(1)}₽)`
)
console.log(
  `      • 8 секунд: $${ourQualityCost8sec} = ${ourQualityCostRub8sec.toFixed(
    0
  )}₽`
)
...
// Сравнение Fast
const fastAdvantage =
  ((SYNX_SELLING_PRICE.price_rub - ourFastCostRub8sec) /
    SYNX_SELLING_PRICE.price_rub) *
  100
const fastMargin = SYNX_SELLING_PRICE.price_rub - ourFastCostRub8sec

console.log(`🔥 Veo-3 Fast (себестоимость) vs Synx (продажная):`)
console.log(
  `   ✅ НАША СЕБЕСТОИМОСТЬ ДЕШЕВЛЕ их продажной цены на ${fastAdvantage.toFixed(
    1
  )}%`
)
console.log(
  `   💰 Маржа: ${fastMargin.toFixed(0)}₽ (${ourFastCostRub8sec}₽ себест. vs ${
    SYNX_SELLING_PRICE.price_rub
  }₽ у них)`
)
...
const qualityAdvantage =
  ((SYNX_SELLING_PRICE.price_rub - ourQualityCostRub8sec) /
    SYNX_SELLING_PRICE.price_rub) *
  100
...
if (qualityAdvantage > 0) {
  console.log(
    `   ✅ НАША СЕБЕСТОИМОСТЬ ДЕШЕВЛЕ их продажной цены на ${qualityAdvantage.toFixed(
      1
    )}%`
  )
  console.log(
    `   💰 Маржа: ${qualityMargin.toFixed(
      0
    )}₽ (${ourQualityCostRub8sec}₽ себест. vs ${
      SYNX_SELLING_PRICE.price_rub
    }₽ у них)`
  )
} else {
  console.log(
    `   ❌ НАША СЕБЕСТОИМОСТЬ ДОРОЖЕ их продажной цены на ${Math.abs(
      qualityAdvantage
    ).toFixed(1)}%`
  )
  console.log(
    `   💸 Убыток: ${Math.abs(qualityMargin).toFixed(
      0
    )}₽ (${ourQualityCostRub8sec}₽ себест. vs ${
      SYNX_SELLING_PRICE.price_rub
    }₽ у них)`
...
margins.forEach(margin => {
  ...
  const vsSync =
    ((SYNX_SELLING_PRICE.price_rub - sellingPrice) /
      SYNX_SELLING_PRICE.price_rub) *
    100
  const status = vsSync > 0 ? '✅ дешевле' : '❌ дороже'
  console.log(
    `   • Наценка ${((margin - 1) * 100).toFixed(0)}%: ${sellingPrice.toFixed(
      0
    )}₽ (${status} Synx на ${Math.abs(vsSync).toFixed(1)}%)`
  )
})

console.log('\n🏆 ВЫВОД:')
console.log(
  `✅ Наша ВХОДЯЩАЯ цена Veo-3 Fast ($${OUR_COST_PRICES['veo-3-fast']}) в ${(
    SYNX_SELLING_PRICE.price_per_second_usd / OUR_COST_PRICES['veo-3-fast']
  ).toFixed(1)}x раза дешевле Synx!`
)
console.log(
  `💰 Огромная маржа для конкурентных цен: до ${fastMargin.toFixed(
    0
  )}₽ прибыли с 8 сек`
)
console.log(
  `🎯 Можем продавать по 35-40₽ и быть дешевле Synx с хорошей прибылью!`
)
