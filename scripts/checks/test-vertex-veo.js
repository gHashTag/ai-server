...
async function getAccessToken() {
  try {
    ...
    console.error(
      `${colors.red}❌ Не удалось получить токен доступа${colors.reset}`
    )
...
async function startVideoGeneration(prompt, aspectRatio = '16:9') {
  ...
  try {
    ...
    console.log(
      `${colors.cyan}📤 Отправляем запрос на генерацию видео...${colors.reset}`
    )
    ...
    console.error(
      `${colors.red}❌ Ошибка при запуске генерации:${colors.reset}`
    )
    if (error.response) {
      ...
      if (error.response.status === 403) {
        ...
        console.log(
          `${colors.cyan}gcloud services enable aiplatform.googleapis.com${colors.reset}`
        )
...
async function waitForCompletion(operationName, maxWaitTime = 300000) {
  ...
  console.log(
    `${colors.cyan}⏳ Ожидаем завершения генерации...${colors.reset}\n`
  )
  ...
  while (Date.now() - startTime < maxWaitTime) {
    ...
    try {
      ...
      process.stdout.write(
        `\r⏳ Попытка ${attempts}: всё ещё обрабатывается...`
      )
      await new Promise(resolve => setTimeout(resolve, checkInterval))
    } catch (error) {
      console.error(
        `\n${colors.red}Ошибка при проверке статуса:${colors.reset}`,
        error.message
      )
...
async function testVeoGeneration() {
  ...
  console.log(
    `${colors.blue}   Vertex AI Veo - Тест генерации видео   ${colors.reset}`
  )
  console.log(
    `${colors.blue}═══════════════════════════════════════════${colors.reset}\n`
  )
  ...
  try {
    ...
    if (stdout.trim()) {
      console.log(
        `${colors.green}✅ Авторизован как: ${stdout.trim()}${colors.reset}\n`
      )
    } else {
  ...
  console.log(
    `${colors.cyan}🎯 Тестовый промпт: "${testPrompts[0]}"${colors.reset}\n`
  )
  ...
  try {
    ...
    if (result.videos && result.videos.length > 0) {
      ...
      result.videos.forEach((video, index) => {
        ...
        if (video.gcsUri) {
          ...
          console.log(
            `  ${colors.cyan}gsutil cp "${video.gcsUri}" ./video_${index}.mp4${colors.reset}`
          )
        }
        if (video.bytesBase64Encoded) {
          console.log(
            `  Base64 закодировано (${video.bytesBase64Encoded.length} символов)`
          )
        }
    ...
    console.error(
      `\n${colors.red}Тест не пройден:${colors.reset}`,
      error.message
    )
  }
}
...
function showInfo() {
  ...
  console.log(
    `   ${colors.cyan}gcloud config set project ${PROJECT_ID}${colors.reset}`
  )
  console.log('\n3. Создайте Cloud Storage bucket для видео:')
  console.log(
    `   ${colors.cyan}gsutil mb gs://veo-videos-${PROJECT_ID}${colors.reset}`
  )
