...
function checkApiKey() {
  ...
  if (process.env.GOOGLE_AI_API_KEY) {
    ...
    console.log(
      `${colors.green}✅ Google AI API ключ найден: ${maskedKey}${colors.reset}`
    )
    return true
  } else {
    console.log(
      `${colors.red}❌ Google AI API ключ не найден в .env${colors.reset}`
    )
    console.log(
      `${colors.yellow}   Добавьте GOOGLE_AI_API_KEY в файл .env${colors.reset}`
    )
    return false
  }
}
...
function checkPricingAndLimits() {
  ...
  veo3Info.models.forEach(model => {
    ...
    console.log(
      `   Доступность: ${colors.yellow}${model.availability}${colors.reset}`
    )
  })
...
async function checkApiAvailability() {
  ...
  try {
      ...
      const req = https.request(options, res => {
        ...
        res.on('end', () => {
          try {
              ...
              if (videoModels.length > 0) {
                ...
                videoModels.forEach(model => {
                  ...
                  if (model.description) {
                    console.log(
                      `     ${colors.yellow}${model.description}${colors.reset}`
                    )
                  }
                })
              } else {
                console.log(
                  `${colors.yellow}⚠️ Модели с поддержкой видео не найдены${colors.reset}`
                )
                console.log(
                  `   Возможно требуется специальный доступ или другой endpoint`
                )
              }
            } else if (res.statusCode === 403) {
              console.log(
                `${colors.red}❌ Ошибка 403: Доступ запрещён${colors.reset}`
              )
              console.log(`   Проверьте правильность API ключа`)
            } else if (res.statusCode === 429) {
              console.log(
                `${colors.yellow}⚠️ Ошибка 429: Превышен лимит запросов${colors.reset}`
              )
            } else {
              console.log(
                `${colors.red}❌ Ошибка ${res.statusCode}${colors.reset}`
              )
              if (response.error) {
            ...
            console.log(
              `${colors.red}❌ Ошибка парсинга ответа: ${e.message}${colors.reset}`
            )
          }
        })
      })
    ...
    console.log(
      `${colors.red}❌ Ошибка при проверке API: ${error.message}${colors.reset}`
    )
  }
}
...
async function main() {
  ...
  console.log(
    `${colors.blue}    Google Veo 3 API - Проверка статуса    ${colors.reset}`
  )
  console.log(
    `${colors.blue}═══════════════════════════════════════════${colors.reset}\n`
  )
