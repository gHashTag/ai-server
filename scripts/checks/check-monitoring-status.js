...
async function checkSystemStatus() {
  ...
  console.log('• Health Check - проверка сервисов каждые 30 минут (cron: */30 * * * *)');
  ...
  if (status.telegram) {
    ...
    const message = `${statusEmoji} <b>Статус системы мониторинга</b>\n\n` +
      `${status.inngest ? '✅' : '❌'} Inngest: ${status.inngest ? 'Работает' : 'Не доступен'}\n` +
      `${status.api ? '✅' : '❌'} API: ${status.api ? 'Работает' : 'Не доступен'}\n` +
      `${status.telegram ? '✅' : '❌'} Telegram: ${status.telegram ? 'Активен' : 'Не доступен'}\n\n` +
      `<b>Активные функции:</b>\n` +
      `• 📊 Log Monitor (24ч)\n` +
      `• 🚨 Critical Error Monitor\n` +
      `• 💚 Health Check (30мин)\n\n` +
      `<i>Проверка выполнена: ${new Date().toLocaleString('ru-RU', { timeZone: 'Europe/Moscow' })}</i>`;
  ...
  if (status.inngest && status.api && status.telegram) {
    ...
    if (!status.inngest) console.log('  - Запустите Inngest: npx inngest-cli@latest dev --port 8288');
