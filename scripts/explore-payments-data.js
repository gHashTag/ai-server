...
async function exploreData() {
  try {
    ...
    if (!income2024Error && incomes2024) {
      ...
      const botsWithIncomes = incomes2024.filter(
        item => item.bot_name && item.bot_name !== null
      )
    ...
    if (!mayJulyError) {
      ...
      if (mayJuly && mayJuly.length > 0) {
        ...
        const completedIncome = mayJuly.filter(
          item => item.status === 'COMPLETED' && item.type === 'MONEY_INCOME'
        )
        console.log(`   - COMPLETED + MONEY_INCOME: ${completedIncome.length}`)

        const completedIncomeWithBots = completedIncome.filter(
          item => item.bot_name && item.bot_name !== null
        )
        console.log(
          `   - COMPLETED + MONEY_INCOME + bot_name: ${completedIncomeWithBots.length}`
        )
        ...
        if (completedIncomeWithBots.length > 0) {
          ...
          completedIncomeWithBots.slice(0, 3).forEach((item, index) => {
            console.log(
              `   ${index + 1}. Bot: ${item.bot_name}, Date: ${
                item.payment_date
              }, Amount: ${item.amount} ${item.currency}, Stars: ${item.stars}`
            )
          })
        }
      }
