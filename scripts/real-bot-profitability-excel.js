...
async function getRealBotProfitability() {
  try {
    ...
    console.log(`❌ Исключено виртуальных/тестовых: ${allPayments.length - realPayments.length}\n`);
...
function displayConsoleReport(botStats) {
  ...
  sortedBots.forEach((stats, index) => {
    ...
    const sortedMonths = Array.from(stats.months.values()).sort((a, b) => a.month.localeCompare(b.month));
    
    if (sortedMonths.length > 0) {
      console.log('   📅 По месяцам:');
      sortedMonths.forEach(month => {
        if (month.rub_income > 0 || month.stars_income > 0) {
          console.log(`   ${month.month}: ${month.rub_income.toFixed(2)} RUB (${month.rub_transactions} тр.) | ${month.stars_income.toFixed(2)} STARS (${month.stars_transactions} тр.)`);
        }
      });
    }

    console.log(`   📊 ИТОГО: ${stats.total_rub.toFixed(2)} RUB | ${stats.total_stars.toFixed(2)} STARS`);
    console.log(`   💎 Условная стоимость: ${(stats.total_rub + stats.total_stars * 0.5).toFixed(2)} RUB`);
  ...
  console.log(`💎 Общая условная стоимость: ${(totalRub + totalStars * 0.5).toFixed(2)} RUB`);
  console.log('='.repeat(80));
}
...
function createExcelReport(botStats, monthlyData) {
  try {
    ...
    const summaryData = [];
    summaryData.push(['БОТ', 'ОБЩИЙ ДОХОД RUB', 'ОБЩИЙ ДОХОД STARS', 'ТРАНЗАКЦИЙ RUB', 'ТРАНЗАКЦИЙ STARS', 'УСЛОВНАЯ СТОИМОСТЬ RUB']);
    ...
    monthlyDetails.push(['БОТ', 'МЕСЯЦ', 'ДОХОД RUB', 'ДОХОД STARS', 'ТРАНЗАКЦИЙ RUB', 'ТРАНЗАКЦИЙ STARS']);
    ...
    transactionData.push(['БОТ', 'ДАТА', 'МЕСЯЦ', 'СУММА RUB', 'ЗВЕЗДЫ', 'ВАЛЮТА', 'МЕТОД ОПЛАТЫ', 'ОПИСАНИЕ', 'ПОДПИСКА']);
