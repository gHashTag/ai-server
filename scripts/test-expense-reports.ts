...
async function testExpenseReports() {
  ...
  try {
    ...
    if (mayExpenses.length > 0) {
      const totalAmount = mayExpenses.reduce(
        (sum, expense) => sum + expense.amount,
        0
      )
      console.log(
        `   Общая сумма: ${totalAmount.toFixed(2)} ${mayExpenses[0].currency}`
      )
    ...
    console.log(
      `   Общая сумма: ${mayStats.totalAmount.toFixed(2)} ${mayStats.currency}`
    )
    ...
    mayStats.categorySummaries.forEach((summary, index) => {
      const percentage = (
        (summary.totalAmount / mayStats.totalAmount) *
        100
      ).toFixed(1)
      console.log(
        `     ${index + 1}. ${summary.category}: ${summary.totalAmount.toFixed(
          2
        )} ${summary.currency} (${percentage}%) - ${summary.count} транзакций`
      )
    })
    ...
    if (aiExpenses.length > 0) {
      const aiTotalAmount = aiExpenses.reduce(
        (sum, expense) => sum + expense.amount,
        0
      )
      console.log(
        `   Общая сумма на AI: ${aiTotalAmount.toFixed(2)} ${
          aiExpenses[0].currency
        }`
      )
    ...
    monthlyStats.forEach(monthStat => {
      const monthName = new Date(monthStat.month + '-01').toLocaleDateString(
        'ru-RU',
        {
          year: 'numeric',
          month: 'long',
      ...
      console.log(
        `     ${monthName}: ${monthStat.totalAmount.toFixed(2)} ${
          monthStat.currency
        } (${monthStat.count} транзакций)`
      )
    })
    console.log()
    ...
    const devExpenses = await getBotFarmExpensesByCategory(
    ...
    if (devExpenses.length > 0) {
      const devTotalAmount = devExpenses.reduce(
        (sum, expense) => sum + expense.amount,
        0
      )
      console.log(
        `   Общая сумма на разработку: ${devTotalAmount.toFixed(2)} ${
          devExpenses[0].currency
        }`
      )
