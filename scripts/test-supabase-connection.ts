...
async function testConnection() {
  ...
  console.log(
    '- SUPABASE_URL:',
    process.env.SUPABASE_URL ? '✅ Установлен' : '❌ Не установлен'
  )
  console.log(
    '- SUPABASE_SERVICE_KEY:',
    process.env.SUPABASE_SERVICE_KEY ? '✅ Установлен' : '❌ Не установлен'
  )
  console.log(
    '- SUPABASE_SERVICE_ROLE_KEY:',
    process.env.SUPABASE_SERVICE_ROLE_KEY ? '✅ Установлен' : '❌ Не установлен'
  )
  console.log()
  ...
  try {
    ...
    if (farmError) {
      console.error(
        '❌ Ошибка при поиске записей фермы ботов:',
        farmError.message
      )
    } else {
