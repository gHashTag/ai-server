  ...
  try {
    ...
    try {
      ...
      topReels.rows.forEach((reel, index) => {
        console.log(`\n${index + 1}. ${reel.username} - ${reel.engagement.toLocaleString()} engagement`)
        console.log(`   👀 Просмотры: ${reel.views ? reel.views.toLocaleString() : 'N/A'}`)
        console.log(`   👍 Лайки: ${reel.likes ? reel.likes.toLocaleString() : 'N/A'}`)
        console.log(`   💬 Коменты: ${reel.comments ? reel.comments.toLocaleString() : 'N/A'}`)
      ...
      console.log(`   📈 Средние подписчики: ${compStats.avg_followers ? Math.round(compStats.avg_followers).toLocaleString() : 'N/A'}`)
      ...
      topCompetitors.rows.forEach((comp, index) => {
        ...
        console.log(`   👥 Подписчики: ${comp.followers_count ? comp.followers_count.toLocaleString() : 'N/A'}`)
        console.log(`   📂 Категория: ${comp.category || 'N/A'}`)
        console.log(`   🔍 Найден для: ${comp.query_user}`)
        if (comp.bio) {
          const shortBio = comp.bio.length > 60 ? comp.bio.substring(0, 60) + '...' : comp.bio
      ...
      if (testUsername) {
        ...
        console.log(`📊 Найдено рилз за последние 14 дней: ${recentReels.rows.length}`)
        
        if (recentReels.rows.length > 0) {
          // Рассчитываем метрики
          const totalViews = recentReels.rows.reduce((sum, reel) => sum + (reel.play_count || 0), 0)
          const totalLikes = recentReels.rows.reduce((sum, reel) => sum + (reel.like_count || 0), 0)
          const totalComments = recentReels.rows.reduce((sum, reel) => sum + (reel.comment_count || 0), 0)
          const avgEngagement = recentReels.rows.reduce((sum, reel) => {
          ...
          recentReels.rows.slice(0, 3).forEach((reel, index) => {
            const engagementRate = ((reel.engagement / Math.max(reel.play_count || 1, 1)) * 100).toFixed(2)
            console.log(`${index + 1}. Engagement: ${reel.engagement.toLocaleString()} (${engagementRate}%)`)
    ...
    console.log('📊 Проблема с RapidAPI не критична - у нас есть реальные данные')
    console.log('💡 Можно демонстрировать полную функциональность на существующих данных')
    console.log('')
