  ...
  try {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(
          `• Reels found: ${statusData.data[0].output?.reelsFound || 0}`
        )

        // Проверяем доставку
        const deliveryResponse = await fetch(
          'http://localhost:8288/v1/events?name=competitor%2Fdelivery-reels'
        )
        const deliveryData = await deliveryResponse.json()

        console.log('\n🎯 РЕЗУЛЬТАТ ТЕСТА:')
        console.log(
          `📬 Найдено delivery событий: ${
        ...
        if (deliveryData.data && deliveryData.data.length > 0) {
          ...
          for (const event of recentEvents) {
            try {
              const deliveryRuns = await fetch(
                `http://localhost:8288/v1/events/${event.id}/runs`
              )
              ...
              if (deliveryRunsData.data && deliveryRunsData.data.length > 0) {
                ...
                if (run.output && run.output.delivery_summary) {
                  console.log(
                    `   • Delivered: ${run.output.delivery_summary.delivered}`
                  )
                  console.log(
                    `   • Failed: ${run.output.delivery_summary.failed}`
                  )
                  console.log(
                    `   • Skipped: ${run.output.delivery_summary.skipped}`
                  )
                }
              }
            } catch (err) {
              console.log(
                `❌ Ошибка проверки delivery ${event.id}:`,
                err.message
              )
            }
          }
