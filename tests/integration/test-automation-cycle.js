...
function simulateAutoParser() {
  ...
  const competitors = [
    ...new Set(mockSubscriptions.map(s => s.competitor_username)),
  ]
...
function simulateDelivery(competitor) {
  ...
  const subscribers = mockSubscriptions.filter(
    s => s.competitor_username === competitor
  )
  ...
  subscribers.forEach((subscriber, index) => {
    const userReels = reels
      .filter(r => r.views_count >= subscriber.min_views)
    ...
    if (subscriber.delivery_format === 'digest') {
      console.log(
        `      - Отправляем дайджест с топ рилсом (${userReels[0]?.views_count} просмотров)`
      )
...
function simulateSystemMonitor() {
  ...
  console.log(
    `      - Доставок: ${stats.deliveries_24h} (успешных: ${stats.successful_deliveries})`
  )

  const healthScore = (stats.successful_deliveries / stats.deliveries_24h) * 100
  console.log(`   💚 Здоровье системы: ${healthScore}%`)
  console.log(
    `   📤 Отчет отправлен админу (${process.env.ADMIN_CHAT_ID || '144022504'})`
  )
}
...
async function runFullCycle() {
  ...
  setTimeout(() => {
    ...
    console.log(
      '1. ✅ competitorAutoParser - автоматический парсинг каждые 24ч'
    )
    console.log(
      '2. ✅ instagramApifyScraper - парсинг через Apify + автотриггер доставки'
    )
