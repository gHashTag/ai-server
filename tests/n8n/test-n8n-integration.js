...
class N8nIntegrationTester {
  ...
  async runTests() {
    ...
    const tests = [
      { name: 'Проверка доступности AI Server', test: () => this.testAiServerHealth() },
      { name: 'Проверка доступности N8N', test: () => this.testN8nHealth() },
      { name: 'Тест N8N API эндпоинтов', test: () => this.testN8nApiEndpoints() },
      { name: 'Тест создания тестового workflow', test: () => this.testWorkflowCreation() },
      { name: 'Тест запуска workflow через webhook', test: () => this.testWorkflowExecution() },
      { name: 'Тест интеграции с Instagram API', test: () => this.testInstagramIntegration() },
      { name: 'Тест интеграции с Generation API', test: () => this.testGenerationIntegration() },
    ];
  ...
  async testN8nApiEndpoints() {
    ...
    for (const endpoint of endpoints) {
      ...
      if (response.status !== endpoint.expectedStatus) {
        throw new Error(`Эндпоинт ${endpoint.path} вернул ${response.status}, ожидался ${endpoint.expectedStatus}`);
      }
  ...
  async testWorkflowCreation() {
    ...
    const response = await this.aiServerClient.post('/api/n8n/workflows', testWorkflow);
  ...
  async testWorkflowExecution() {
    ...
    try {
      const response = await axios.post(`${config.n8nUrl}/webhook/test-integration`, testData, {
        timeout: 10000
      });
  ...
  async testInstagramIntegration() {
    ...
    try {
      const response = await this.aiServerClient.post('/api/inngest/instagram-scraper', testData, {
        headers: {
  ...
  async testGenerationIntegration() {
    ...
    try {
      const response = await this.aiServerClient.post('/api/generation/neuro-image', testData, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
