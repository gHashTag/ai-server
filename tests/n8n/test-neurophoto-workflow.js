...
class NeuroPhotoWorkflowTester {
  ...
  async testValidRequest() {
    const testData = {
      prompt:
        'Beautiful portrait of a person in a magical forest, high quality, detailed',
    ...
    if (!data.success) {
      throw new Error(
        `Workflow returned success=false: ${JSON.stringify(data)}`
      )
    }
  ...
  async testMissingPrompt() {
    ...
    if (
      ...
      !data.validation_error.missing_fields.includes('prompt')
    ) {
  ...
  printTestSummary() {
    ...
    const criticalFailed = this.testResults.filter(
    ...
    this.testResults.forEach((result, index) => {
      ...
      console.log(
        `   ${index + 1}. ${icon} ${result.name}${critical}${duration}`
      )
