...
class N8nController {
  ...
  public receiveWebhook = async (
    ...
    try {
      ...
      const result = await this.n8nService.processWebhookData(
        workflowId,
        executionId,
        data
      )
  ...
  public triggerWorkflow = async (
    ...
    try {
      ...
      const executionId = await this.n8nService.triggerWorkflow(
        workflowName,
        data
      )
