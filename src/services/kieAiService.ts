...
export class KieAiService {
  ...
  async generateVideo(options: KieAiGenerationOptions): Promise<{
    ...
    if (model === 'veo3_fast') {
      ...
      if (duration !== 8) {
        console.log(
          `🚨 CRITICAL: Veo 3 Fast supports ONLY 8 seconds! Forced duration from ${duration}s to 8s`
        )
      }
    } else {
    ...
    try {
      ...
      while (attempts < maxAttempts && !videoUrl) {
        attempts++
        console.log(
          `🔍 Проверка статуса (попытка ${attempts}/${maxAttempts})...`
        )
        ...
        try {
          ...
          if (statusResponse.data.code === 200 && statusResponse.data.data) {
            ...
            if (taskData.status === 'completed' && taskData.videoUrl) {
              ...
              throw new Error(
                `Video generation failed: ${taskData.error || 'Unknown error'}`
              )
            }
      ...
      if (!videoUrl) {
        ...
        throw new Error(
          `Video generation timeout after ${
            maxWaitTime / 1000
