...
export class GitHubWebhookService {
  ...
  constructor() {
    ...
    if (!this.githubToken) {
      logger.warn('⚠️ GITHUB_TOKEN не установлен - некоторые функции будут недоступны');
    }
  }
  ...
  public async analyzePRForAutoFix(prInfo: any): Promise<any> {
    ...
    try {
      ...
      const prDetails = await this.getPRDetails(prInfo.repository, prInfo.number);
      
      if (!prDetails || prDetails.state !== 'open') {
        logger.info('📋 PR закрыт или не найден, пропускаем', { pr: prInfo.number });
        return { action: 'skipped', reason: 'pr_not_open' };
      }

      // Шаг 2: Получить failed checks
      const failedChecks = await this.getFailedChecks(prInfo.repository, prInfo.number);
      ...
      logger.info(`🚨 Найдено ${failedChecks.length} проблем в PR #${prInfo.number}`, {
      ...
      const changedFiles = await this.getPRFiles(prInfo.repository, prInfo.number);
      ...
      if (!claudeAnalysis || !claudeAnalysis.fixes) {
        logger.info('🤔 Claude Code не предложил исправлений', { pr: prInfo.number });
   ...
   */
  private async getPRDetails(repository: string, prNumber: number): Promise<any> {
    try {
      ...
      logger.error('Ошибка получения информации о PR', { repository, prNumber, error: error.message });
      return null;
    }
  }
   ...
   */
  private async getFailedChecks(repository: string, prNumber: number): Promise<any[]> {
    try {
      const result = execSync(
      ...
      logger.warn('Не удалось получить статус проверок PR', { repository, prNumber });
      return [];
    }
  }
  ...
  private async getPRDiff(repository: string, prNumber: number): Promise<string> {
    try {
      return execSync(
        `gh pr diff ${prNumber} --repo ${repository}`,
        { encoding: 'utf8' }
      );
    } catch (error) {
   ...
   */
  private async getPRFiles(repository: string, prNumber: number): Promise<string[]> {
    try {
      const result = execSync(
      ...
      logger.warn('Не удалось получить список файлов PR', { repository, prNumber });
      return [];
    }
  }
...
${context.changed_files.slice(0, 15).map(f => `- ${f}`).join('\n')}
${context.changed_files.length > 15 ? `... and ${context.changed_files.length - 15} more files` : ''}

**Diff Preview:**
\`\`\`diff
${context.diff.length > 3000 ? context.diff.slice(0, 3000) + '\n... (truncated)' : context.diff}
\`\`\`
    ...
    try {
      ...
      for (const fix of fixes) {
        try {
          ...
          logger.warn(`⚠️ Не удалось применить исправление для ${fix.file}`, { error: error.message });
        }
      }

      if (appliedFixes === 0) {
        return { success: false, error: 'No fixes were applied', fixes_applied: 0 };
      }

      // Создать коммит с исправлениями
      execSync('git add .');
      
      const commitMessage = options.commit_message || 
