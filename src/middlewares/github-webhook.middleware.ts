...
export class GitHubWebhookMiddleware {
  ...
  constructor() {
    this.webhookSecret =
      process.env.GITHUB_WEBHOOK_SECRET || 'default-webhook-secret'

    if (this.webhookSecret === 'default-webhook-secret') {
      logger.warn(
        '⚠️ GITHUB_WEBHOOK_SECRET не установлен - используется значение по умолчанию'
      )
    }
  }
