import { createClient } from '@supabase/supabase-js'
import {
  SUPABASE_URL,
  SUPABASE_SERVICE_KEY,
  SUPABASE_SERVICE_ROLE_KEY,
} from '../../config'

// Создаем клиент с service role key
export const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY)

export const supabaseAdmin = createClient(
  SUPABASE_URL,
  SUPABASE_SERVICE_ROLE_KEY
)

export * from './createModelTraining'
export * from './checkSubscriptionByTelegramId'
export * from './updateUserBalance'
export * from './getAspectRatio'
export * from './getGeneratedImages'
export * from './getHistory'
export * from './getModel'
export * from './getPrompt'
export * from './getUserData'
export * from './getUserDataV2'
export * from './getUserByTelegramIdV2'
export * from './incrementGeneratedImages'
export * from './isLimitAi'
export * from './savePrompt'
export * from './setAspectRatio'
export * from './getUid'
export * from './getUserBalance'
export * from './updateUserVoice'
export * from './updateUserVoiceV2'
export * from './getUserModel'
export * from './getReferalsCount'
export * from './cleanupOldArchives'
export * from './deleteFileFromSupabase'
export * from './ensureSupabaseAuth'
export * from './getTelegramIdByUserId'
export * from './getVoiceId'
export * from './saveUserEmail'
export * from './sendPaymentInfo'
export * from './sendPaymentInfoV2'
export * from './getPaymentsInfoByUsername'
export * from './getPaymentsInfoByUsernameV2'
export * from './getPaymentsInfoByTelegramId'
export * from './getPaymentsInfoByTelegramIdV2'
export * from './setModel'
export * from './updateUserSoul'
export * from './getUserByTelegramId'
export * from './incrementBalance'
export * from './saveFileLinkToSupabase'
export * from './setSyncLabsVideo'
export * from './updateResult'
export * from './setPayments'
export * from './setPaymentsV2'
export * from './checkUsernameCodesByUserId'
export * from './createUser'
export * from './getSelectIzbushkaId'
export * from './setMyWorkspace'
export * from './setPassport'
export * from './getAiFeedbackFromSupabase'
export * from './getLastStep'
export * from './getPlan'
export * from './updateHistory'
export * from './createHistory'
export * from './setLeelaStart'
export * from './updateUser'
export * from './updateUserLevelPlusOne'
export * from './getTelegramIdFromInvId'
export * from './getRoomById'
export * from './tasks'
export * from './room_assets'
export * from './workspaces'
export * from './getModelTrainingData'
export * from './getFineTuneIdByTelegramId'
export * from './updatePrompt'
export * from './updateLatestModelTraining'
export * from './createModelTrainingV2'
export * from './getModelByTelegramId'
export * from './updateUserSubscription'
export * from './getTaskData'
export * from './updateLatestModelTrainingQuick'
export * from './updateUserBalance'
export * from './getUserBalance'
export * from './getSubScribeChannel'
export * from './updateUserBalanceRobokassa'
export * from './notifyBotOwners'
export * from './getUserDetailsSubscription'
export * from './updateUserBalanceRobokassaV2'
export * from './updateUserSubscriptionV2'
export * from './getPaymentHistoryV2'
export * from './getPromptHistoryV2'
export * from './wrappers'
