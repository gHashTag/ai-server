 ...
 */
export async function notifyDeploymentStart(
  ...
  try {
    ...
    message +=
      '⏳ Автоматические проверки network check начнутся через 2 минуты после завершения\n'
    message += '\n#deployment #started'

    await bot.api.sendMessage(process.env.ADMIN_CHAT_ID!, message)

    logger.info('Deployment start notification sent', {
  ...
  while (Date.now() - startTime < maxWaitMs) {
    try {
      ...
      if (healthResponse.ok) {
        return {
          healthy: true,
          message: `Система стала доступна через ${Math.round(
