...
const TEST_SCENARIOS = [
  {
    ...
    model: 'veo3_fast',
    prompt:
      'Beautiful sunset over ocean waves, cinematic vertical shot for social media',
    duration: 3,
    priority: 'CRITICAL',
  },
  {
    ...
    model: 'veo3_fast',
    prompt:
      'Epic mountain landscape with flying eagle, wide cinematic shot for YouTube',
    duration: 4,
    priority: 'HIGH',
  },
  {
    ...
    model: 'veo3_fast',
    prompt:
      'Close-up of blooming flower in garden, square format for Instagram feed',
    duration: 3,
    priority: 'MEDIUM',
  },
]
...
class VEO3FormatTester {
  ...
  async testVideoGeneration(scenario) {
    ...
    try {
      ...
      if (finalResult.success) {
        ...
        await this.validateVideoFormat(
      ...
      try {
        ...
        if (response.data.code === 200 && response.data.data) {
          ...
          if (data.status === 'completed' && data.videoUrl) {
            ...
            console.log(
              `   ⏳ Статус: ${data.status}... (${attempt}/${maxAttempts})`
            )
          }
        }
        ...
        console.error(
          `   ⚠️ Ошибка при проверке статуса (попытка ${attempt}):`,
          error.message
        )
        if (attempt < maxAttempts) {
  ...
  generateReport() {
    ...
    this.results.forEach((result, index) => {
      ...
      console.log(
        `${priority} ${status} ${result.aspectRatio} - ${result.scenario}`
      )
