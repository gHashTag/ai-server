...
async function checkHealthFixed() {
  ...
  try {
    const response = await axios.get(
      `${KIE_AI_CONFIG.BASE_URL}${CORRECT_CONFIG.endpoints.chat_credit}`,
      {
...
async function testVeoQualityGeneration() {
  ...
  const testPayload = {
    prompt:
      'A professional dancer performing in a modern studio, vertical composition for Instagram',
  ...
  try {
    ...
    console.error(
      `❌ veo3 тест неудачен: ${error.response?.status || error.message}`
    )
    if (error.response?.data) {
      console.error(`📋 Детали:`, JSON.stringify(error.response.data, null, 2))
    }
...
async function main() {
  ...
  console.log(
    `🎯 9:16 veo3_fast: ${
      results.tests.vertical_9x16_fast?.success
        ? '✅ РАБОТАЕТ'
        : '❌ НЕ РАБОТАЕТ'
    }`
  )
  console.log(
    `🎨 9:16 veo3: ${
      results.tests.vertical_9x16_quality?.success
        ? '✅ РАБОТАЕТ'
        : '❌ НЕ РАБОТАЕТ'
    }`
  )
  ...
  if (
    ...
    console.log(
      '   3. Проверьте актуальность model identifiers в документации Kie.ai'
    )
  }
