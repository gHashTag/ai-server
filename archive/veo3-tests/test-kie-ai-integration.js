...
async function testProcessVideoGenerationIntegration() {
  ...
  try {
    ...
    const checks = [
      {
        item: 'KieAiService',
        text: 'import { KieAiService }',
        description: 'Импорт KieAiService',
      },
      {
        item: 'type check',
        text: "providerType === 'kie-ai'",
        description: 'Проверка типа провайдера',
      },
      {
        item: 'fallback',
        text: 'processVertexAI',
        description: 'Fallback функция',
      },
      {
        item: 'duration param',
        text: 'duration:',
        description: 'Передача duration',
      },
      {
        item: 'health check',
        text: 'checkHealth',
    ...
    if (
      content.includes('Using Kie.ai for') &&
      content.includes('cheaper than Vertex AI')
    ) {
...
async function testEnvironmentSetup() {
  ...
  const envVars = [
    { name: 'KIE_AI_API_KEY', required: true, description: 'Ключ Kie.ai API' },
    {
      ...
      required: false,
      description: 'Проект Google Cloud (для fallback)',
    },
  ]
  ...
  envVars.forEach(envVar => {
    ...
    if (value) {
      ...
      console.log(
        `   ${envVar.required ? '❌' : '⚠️'} ${envVar.name}: ${
          envVar.required ? 'отсутствует' : 'не настроен'
        }`
      )
