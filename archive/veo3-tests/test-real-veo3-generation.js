...
async function testRealVeo3Generation() {
  ...
  try {
    ...
    try {
      ...
      console.error('❌ Альтернативный способ тоже не работает:', fallbackError.message)
    }
  }
  ...
  try {
    ...
    console.log('✅ Горизонтальное видео запущено:', response.status, response.data)
  ...
  for (const model of availableModels) {
    try {
      ...
      console.log(`   ❌ ${model} не работает:`, error.response?.status || error.message)
    }
  }
  ...
  for (const endpoint of webhookEndpoints) {
    try {
      ...
      if (error.response?.status === 405) {
        console.log(`✅ ${endpoint}: доступен (METHOD NOT ALLOWED - это нормально для webhook)`)
      } else {
