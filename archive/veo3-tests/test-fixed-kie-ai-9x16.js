...
async function checkHealthFixed() {
  ...
  try {
    const response = await axios.get(
      `${KIE_AI_CONFIG.BASE_URL}${CORRECT_ENDPOINTS.health}`,
      {
    ...
    try {
      ...
      const creditResponse = await axios.get(
        `${KIE_AI_CONFIG.BASE_URL}${CORRECT_ENDPOINTS.chat_credit}`,
        {
          headers: KIE_AI_CONFIG.HEADERS,
          timeout: 10000,
        }
      ...
      console.error(
        `❌ Credit check тоже неудачен: ${
          creditError.response?.status || creditError.message
        }`
      )
      return false
    }
...
async function testVeo9x16Generation() {
  ...
  const testPayload = {
    prompt:
      'Beautiful sunset over ocean waves, perfect vertical shot for social media',
    model: 'veo-3-fast',
    duration: 3,
    aspectRatio: '9:16',
    resolution: '720p',
  ...
  try {
    ...
    console.error(
      `❌ Ошибка генерации 9:16: ${error.response?.status || error.message}`
    )
...
async function testVeo16x9Generation() {
  ...
  try {
    ...
    console.error(
      `❌ 16:9 тест неудачен: ${error.response?.status || error.message}`
    )
...
async function main() {
  ...
  console.log(
    `🏥 Health Check: ${
      results.tests.health ? '✅ Работает' : '❌ Не работает'
    }`
  )
  console.log(
    `🎯 Вертикальное 9:16: ${
      results.tests.vertical_9x16?.success ? '✅ РАБОТАЕТ' : '❌ НЕ РАБОТАЕТ'
    }`
  )
  console.log(
    `📺 Горизонтальное 16:9: ${
      results.tests.horizontal_16x9?.success ? '✅ Работает' : '❌ Не работает'
    }`
  )
  ...
  if (results.tests.vertical_9x16?.success) {
    ...
    console.log(
      '💡 Теперь нужно перезапустить сервер для применения исправлений'
    )
  } else {
...
module.exports = {
  checkHealthFixed,
  testVeo9x16Generation,
  testVeo16x9Generation,
}
