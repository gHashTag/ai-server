...
const VIDEO_MODELS_CONFIG = {
  'veo-3-fast': {
    ...
    description:
      '⚡ Супер быстрая генерация через Kie.ai (дешевле Synx!), 2-8 сек',
...
async function testKieAiIntegration() {
  ...
  kieAiModels.forEach(([modelId, config]) => {
    ...
    console.log(
      `      • Цена за секунду: $${
        config.pricePerSecond?.toFixed(3) || 'dynamic'
      }`
    )
    console.log(`      • Описание: ${config.description}`)
    console.log(
      `      • Поддерживаемые длительности: ${
        config.supportedDurations?.join(', ') || 'any'
      }`
    )
  })
  ...
  const VERTEX_AI_PRICES = {
    'veo-3-fast': 0.3, // Vertex AI fast
    'veo-3': 0.4, // Vertex AI quality
  }
  ...
  Object.keys(KIE_AI_PRICES).forEach(model => {
    ...
    if (vertexPrice && kiePrice) {
      ...
      console.log(
        `   💰 Экономия: ${savings.toFixed(1)}% (в ${ratio.toFixed(
          1
        )}x дешевле!)`
      )
  ...
  console.log(
    `📋 KIE_AI_API_KEY: ${hasKieApiKey ? '✅ Настроен' : '❌ Отсутствует'}`
  )
  console.log(
    `📋 GOOGLE_CLOUD_PROJECT: ${
      hasVertexProject ? '✅ Настроен' : '❌ Отсутствует'
    }`
  )
